import { fallbackMovieImage, getOnAirTvShows, getOnAirTvShowsPage, image500 } from "@/api/db";
...
const OnAirTvSeries = () => {
    ...
    const [onAirTvSeries, setOnAirTvSeries] = useState<Serie[]>([]);
    const [page, setPage] = useState<number>(1);

    useEffect(() => {
        getOnAirTvShows().then((data) => {
            setOnAirTvSeries(data);
            setLoading(false);
        });
    }, []);

    const loadMore = () => {
        getOnAirTvShowsPage(page + 1).then((data) => {
            setOnAirTvSeries([...onAirTvSeries, ...data]);
            setPage(page + 1);
        });
    };
        ...
        <View className="flex-1 bg-neutral-900">
            ...
            >
                ...
                <Text className="text-4xl font-bold text-white">
                    <Text style={{ color: "#eab308" }}>On </Text>Air
                </Text>
                <TouchableOpacity
